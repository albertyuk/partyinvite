...
async function setup(browser) {
  ...
  await ctx.route('**/*', async (route) => {
    ...
    if (process.env.BLOCK_VIDEO && /\.(mp4|webm|mov|m4v)(\?|$)/i.test(url)) { try { await route.abort(); } catch {} return; }
    try {
...
async function pageData(page) {
  return await page.evaluate(() => {
    ...
    return {
      ...
      blankNoopener: (() => { const bad = [...document.querySelectorAll('a[target="_blank"]')].filter(a => !/(noopener|noreferrer)/i.test(a.rel || '')); return { count: bad.length, examples: bad.slice(0, 5).map(a => (a.href || '').slice(0, 120)) }; })(),
      dupIds: (() => { const seen = {}; document.querySelectorAll('[id]').forEach(el => { seen[el.id] = (seen[el.id] || 0) + 1; }); const dups = Object.entries(seen).filter(([, n]) => n > 1); return { count: dups.length, examples: dups.slice(0, 6).map(([id, n]) => `${id} x${n}`) }; })(),
      deprecatedTags: document.querySelectorAll('font, center, marquee, blink, big').length,
    };
  });
}
...
(async () => {
  ...
  while (queue.length && rendered < MAX_RENDER) {
    ...
    // survive a mid-crawl browser crash: recreate the page and continue
    if (page.isClosed()) { try { page = await ctx.newPage(); } catch { break; } }
    ...
    if (nav.ok) {
      ...
      if (Array.isArray(d.forms) && d.forms.length) out.forms.push({ page: item.url, forms: d.forms });
        ...
        rec.extractError = String(e.message || e).split('\n')[0];
        try { if (page.isClosed()) page = await ctx.newPage(); } catch {}
      }
    }
